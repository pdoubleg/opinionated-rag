...
path_splade = Path(os.path.join("data", "splade", "splade_embeddings.npz"))
...
class SparseEmbeddingsSplade:
    ...
    def _get_batch_embeddings(
        self, docs: List[str], free_memory: bool = False
    ) -> np.ndarray:
        """
        Generates embeddings for a batch of documents.

        Args:
            docs (List[str]): The documents to generate embeddings for.
            free_memory (bool, optional): Whether to free memory after generating embeddings. Defaults to False.
    ...
    def _get_embedding_fnames(self):
        folder_name = os.path.join(path_splade, "splade")
    ...
    def load(self) -> None:
        """
        Loads embeddings, ids, and metadata from disk.

        Raises:
            FileNotFoundError: If the embeddings, ids, or metadata files do not exist on disk.
        """
    ...
    def generate_embeddings_from_docs(
        ...
        ids = [d.metadata["id_"] for d in docs]
    ...
    def query(
        self, search: str, chunk_size: int, n: int = 50, label: str | None = None
