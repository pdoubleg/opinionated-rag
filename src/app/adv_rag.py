...
class DocElements(BaseModel):
    ...
    def text_elements_to_markdown(self) -> str:
        """
        Formats the text document elements as a single Markdown string.

        Returns:
            str: The concatenated Markdown string of all text document elements.
        """
        return "\n\n".join([element.to_markdown() for element in self.text_doc_elements])

    def table_elements_to_markdown(self) -> str:
        """
        Formats the table document elements as a single Markdown string.

        Returns:
            str: The concatenated Markdown string of all table document elements.
        """
        return "\n\n".join([element.to_markdown() for element in self.table_doc_elements])
        ...
        if filepath is not None:
            ...
            if "retriever" not in st.session_state:
                ...
                with st.expander("Doc elements"):
                    text_markdown = doc_elements.text_elements_to_markdown()
                    table_markdown = doc_elements.table_elements_to_markdown()
                    st.markdown("Text elements")
                    st.markdown(text_markdown, unsafe_allow_html=True)
                    st.markdown("Table elements")
