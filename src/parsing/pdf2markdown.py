...
Text will be sorted in Western reading order. Tables will be included in
the text in markdown format.

Depends on PyMuPDF v1.24.0 or later
...
def convert_pdf_to_markdown(filename: str, pages_spec: str = None) -> str:
    """
    Convert a PDF file to a markdown string.

    Args:
        filename (str): The path to the PDF file.
        pages_spec (str, optional): A string specifying the pages to convert.
            Format: "start-end,page1,page2,...". Defaults to None, which converts all pages.
    ...
    if pages_spec:
        ...
        pages_spec = pages_spec.replace("N", f"{doc.page_count}")
        ...
        if wrong_pages != set():  # if any invalid numbers given, raise an exception
            raise ValueError(f"Page number(s) {wrong_pages} not in '{doc}'.")
