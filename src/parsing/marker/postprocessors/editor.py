...
from src.parsing.marker.postprocessors.t5 import T5ForTokenClassification, byt5_tokenize
...
def edit_full_text(text: str, model: Optional[T5ForTokenClassification], batch_multiplier=1) -> (str, dict):
    if not settings.ENABLE_EDITOR_MODEL:
        return text, dict(delete=0, newline=0, space=0, equal=0)
