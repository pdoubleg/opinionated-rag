...
from src.cachedb.redis_cachedb import RedisCacheConfig
from src.llm.base import LLMMessage, Role
from src.llm.openai_gpt import (
    AccessWarning,
    OpenAIChatModel,
    OpenAIGPT,
    OpenAIGPTConfig,
)
from src.parsing.parser import Parser, ParsingConfig
from src.parsing.utils import generate_random_sentences
from src.utils.configuration import Settings, set_global
...
def _test_context_length_error(test_settings: Settings, mode: str, max_tokens: int):
    ...
    cfg = OpenAIGPTConfig(
        ...
        model=OpenAIChatModel.GPT4,  # or GPT4_TURBO,
        cache_config=RedisCacheConfig(fake=True),
    )
...
def test_model_selection(test_settings: Settings):
    ...
    defaultOpenAIChatModel = llm.openai_gpt.defaultOpenAIChatModel
    ...
    llm.openai_gpt.defaultOpenAIChatModel = OpenAIChatModel.GPT4_TURBO
    llm = OpenAIGPT(config=OpenAIGPTConfig(model=OpenAIChatModel.GPT3_5_TURBO))
    ...
    llm.openai_gpt.defaultOpenAIChatModel = OpenAIChatModel.GPT3_5_TURBO
    ...
    llm = OpenAIGPT(config=OpenAIGPTConfig(model="local/localhost:8000"))
    check_warning(llm, False, function=simulate_response)
    llm = OpenAIGPT(config=OpenAIGPTConfig(model="litellm/ollama/llama"))
    check_warning(llm, False, function=simulate_response)
    ...
    llm.openai_gpt.defaultOpenAIChatModel = defaultOpenAIChatModel