...
def screen_query(query: str, model_name: str = 'gpt-3.5-turbo') -> QueryScreening:
    ...
    return client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are helpful AI assistant for a legal research application. Your task is to perform an initial screening on a newly received user query using the correct format.",
            },
            {
                "role": "user",
...
class QueryFilterPlan(BaseModel):
    ...
    filter: Optional[str] | None = Field(
        None, description="An SQL-like filter inferred from the user query."
    )
    ...
    def filter_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result_df = table.search().where(self.filter).limit(None).to_df()
        db.drop_database()
...
def generate_query_plans(
    ...
    return client.chat.completions.create(
        model="gpt-4-1106-preview",
        response_model=Iterable[QueryFilterPlan],
        max_retries=Retrying(
            stop=stop_after_attempt(5),
            wait=wait_fixed(1),
        ),
        messages=[
            ...
            {
                "role": "user",
                "content": f"Please decompose the user's query into **{n_subquestions}** distinct sub-questions, and generate a query plan for each. Here is the QUERY: {query}",
            },
        ],
    )
...
def auto_filter_vector_search(
    ...
    if use_llm_filter and query_plan.filter:
        result = (
            table.search(np.array(query_vector))
            .metric("cosine")
        ...
        result = (
            table.search(np.array(query_vector)).metric("cosine").limit(top_k).to_df()
        )
...
def auto_filter_fts_search(
    ...
    if use_llm_filter and query_plan.filter:
        ...
        result = table.search(query_clean, query_type="fts").limit(top_k).to_df()
    result.rename(columns={"vector": embeddings_column}, inplace=True)
    logger.info(
